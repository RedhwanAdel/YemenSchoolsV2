...
{
    public class SchoolRepository : GenericRepositoryAsync<School>, ISchoolRepository
    {
        private readonly YemenShoolsDbContext dbContext;

        public SchoolRepository(YemenShoolsDbContext dbContext) : base(dbContext)
        {
       ...
       .FirstOrDefaultAsync();
        ...
        public async Task AssignSubjectsToSchoolGradeAsync(Guid schoolGradeId, List<Guid> subjectIds)
        {
            ...
            var existingAssignments = await dbContext.GradeSubjects
                                .AsNoTracking()
                                .Where(gs => gs.SchoolGradeId == schoolGradeId)
                                .ToListAsync();

            dbContext.GradeSubjects.RemoveRange(existingAssignments.Where(ea => !subjectIds.Contains(ea.SubjectId)));
            ...
            {
                dbContext.GradeSubjects.Add(new GradeSubject
                {
        ...
        {
            return await dbContext.GradeSubjects
                                 .Where(gs => gs.SchoolGradeId == schoolGradeId)
        ...
        public async Task<SchoolPhoto> AddSchoolPhotoAsync(SchoolPhoto schoolPhoto)
        {
            await dbContext.SchoolPhotos.AddAsync(schoolPhoto);
            await dbContext.SaveChangesAsync();
            return schoolPhoto;
        }

        public async Task<List<SchoolPhoto>> GetSchoolPhotosAsync(Guid schoolId)
        {
            return await dbContext.SchoolPhotos
                .AsNoTracking()
